...
#include "vulkan/shader_cache.h"
#include "vulkan/stream_buffer.h"
#include "vulkan/swap_chain.h"
#include "vulkan/util.h"
#include <array>
Log_SetChannel(VulkanGPUDevice);

VulkanGPUDevice::VulkanGPUDevice() = default;
...
RenderAPI VulkanGPUDevice::GetRenderAPI() const
{
...
bool VulkanGPUDevice::SupportsFullscreen() const
{
  return false;
}

bool VulkanGPUDevice::IsFullscreen()
{
  return false;
}

bool VulkanGPUDevice::SetFullscreen(bool fullscreen, u32 width, u32 height, float refresh_rate)
{
  return false;
}
...
void VulkanGPUDevice::EndTextureUpdate(GPUTexture* texture, u32 x, u32 y, u32 width, u32 height)
{
  static_cast<Vulkan::Texture*>(texture)->EndUpdate(x, y, width, height, 0, 0);
}

bool VulkanGPUDevice::UpdateTexture(GPUTexture* texture, u32 x, u32 y, u32 width, u32 height, const void* data,
                                    u32 pitch)
{
...
bool VulkanGPUDevice::SetupDevice()
{
  if (!CreateResources())
    return false;
...
VkRenderPass VulkanGPUDevice::GetRenderPassForDisplay() const
{
  if (m_swap_chain)
  {
...
void VulkanGPUDevice::DestroyStagingBuffer()
{
...
bool VulkanGPUDevice::DownloadTexture(GPUTexture* texture, u32 x, u32 y, u32 width, u32 height, void* out_data,
                                      u32 out_data_stride)
{
...
bool VulkanGPUDevice::CheckStagingBufferSize(u32 required_size)
{
...
void VulkanGPUDevice::DestroyResources()
{
...
void VulkanGPUDevice::DestroyImGuiContext()
{
  g_vulkan_context->WaitForGPUIdle();
  ImGui_ImplVulkan_Shutdown();
}

bool VulkanGPUDevice::UpdateImGuiFontTexture()
{
...
bool VulkanGPUDevice::DoneCurrent()
{
  return true;
}
...
void VulkanGPUDevice::RenderDisplay(s32 left, s32 top, s32 width, s32 height, Vulkan::Texture* texture,
...
void VulkanGPUDevice::RenderImGui()
{
...
void VulkanGPUDevice::RenderSoftwareCursor()
{
...
void VulkanGPUDevice::RenderSoftwareCursor(s32 left, s32 top, s32 width, s32 height, GPUTexture* texture)
{
  VkCommandBuffer cmdbuffer = g_vulkan_context->GetCurrentCommandBuffer();
...
float VulkanGPUDevice::GetAndResetAccumulatedGPUTime()
{
  return g_vulkan_context->GetAndResetAccumulatedGPUTime();
}

GPUDevice::AdapterAndModeList VulkanGPUDevice::StaticGetAdapterAndModeList(const WindowInfo* wi)
{
...
VulkanGPUDevice::PostProcessingStage::~PostProcessingStage()
{
...
void VulkanGPUDevice::ApplyPostProcessingChain(VkFramebuffer target_fb, s32 final_left, s32 final_top, s32 final_width,
                                               s32 final_height, Vulkan::Texture* texture, s32 texture_view_x,
