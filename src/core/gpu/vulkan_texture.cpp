...
void VulkanTexture::TransitionToLayout(VkCommandBuffer command_buffer, Layout new_layout)
{
  ...
  TransitionSubresourcesToLayout(command_buffer, 0, m_levels, m_layout, new_layout);

  m_layout = new_layout;
}

void VulkanTexture::TransitionSubresourcesToLayout(VkCommandBuffer command_buffer, u32 start_level, u32 num_levels,
                                                   Layout old_layout, Layout new_layout)
{
  TransitionSubresourcesToLayout(command_buffer, m_image, m_type, start_level, num_levels, old_layout, new_layout);
}
                                                   ...
                                                   Layout new_layout)
{
  ...
  VkImageMemoryBarrier barrier = {VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER,
                                  ...
                                  VK_QUEUE_FAMILY_IGNORED,
                                  image,
                                  {aspect, start_level, num_levels, 0u, 1u}};
                                   ...
                                   u32 out_data_stride)
{
  ...
  if (old_layout != VulkanTexture::Layout::TransferSrc)
    T->TransitionSubresourcesToLayout(cmdbuf, 0, 1, old_layout, VulkanTexture::Layout::TransferSrc);
  ...
  if (old_layout != VulkanTexture::Layout::TransferSrc)
    T->TransitionSubresourcesToLayout(cmdbuf, 0, 1, VulkanTexture::Layout::TransferSrc, old_layout);
