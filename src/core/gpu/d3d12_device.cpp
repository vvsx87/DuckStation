...
ID3D12GraphicsCommandList4* D3D12Device::GetInitCommandList()
{
  ...
  if (!res.init_list_used)
  {
    ...
    DebugAssertMsg(SUCCEEDED(hr), "Reset init command allocator failed");

    res.command_lists[0]->Reset(res.command_allocators[0].Get(), nullptr);
    DebugAssertMsg(SUCCEEDED(hr), "Reset init command list failed");
...
void D3D12Device::WaitForFence(u64 fence)
{
  ...
  if (m_completed_fence_value < fence)
  {
    ...
    DebugAssertMsg(SUCCEEDED(hr), "Set fence event on completion");
...
bool D3D12Device::CreateSwapChain()
{
  ...
  LookupNativeFormat(s_swap_chain_format, &dxgi_format, nullptr, nullptr, nullptr);
...
std::string D3D12Device::GetShaderCacheBaseName(const std::string_view& type) const
{
  return fmt::format("d3d12_{}{}", type, m_debug_device ? "_debug" : "");
}
...
bool D3D12Device::SupportsTextureFormat(GPUTexture::Format format) const
{
  ...
  DXGI_FORMAT dxgi_format;
  LookupNativeFormat(format, &dxgi_format, nullptr, nullptr, nullptr);

  D3D12_FEATURE_DATA_FORMAT_SUPPORT support = {dxgi_format};
                      ...
                      const std::array<float, 3>& c, const std::array<float, 3>& d)
{
  ...
  return PIX_COLOR(static_cast<BYTE>(result[0]), static_cast<BYTE>(result[1]), static_cast<BYTE>(result[2]));
}
#endif
...
void D3D12Device::SetTextureSampler(u32 slot, GPUTexture* texture, GPUSampler* sampler)
{
  ...
  const D3D12DescriptorHandle& handle = sampler ? static_cast<D3D12Sampler*>(sampler)->GetDescriptor() : m_point_sampler;
