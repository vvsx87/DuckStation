...
u32 VulkanDevice::GetActiveTexturesForLayout(GPUPipeline::Layout layout)
{
  static constexpr std::array<u8, static_cast<u8>(GPUPipeline::Layout::MaxCount)> counts = {
...
bool VulkanDevice::CreateDevice(const std::string_view& adapter)
{
  ...
  bool threaded_presentation = false; // TODO: Config
...
std::string VulkanDevice::GetShaderCacheBaseName(const std::string_view& type) const
{
  return fmt::format("vulkan_{}{}", type, m_debug_device ? "_debug" : "");
}
...
{
#if 0
  VulkanTexture* sTexVK = static_cast<VulkanTexture*>(sTex);
  VulkanTexture* dTexVK = static_cast<VulkanTexture*>(dTex);

  EndRenderPass();

  sTexVK->TransitionToLayout(VulkanTexture::Layout::TransferSrc);
  dTexVK->TransitionToLayout(VulkanTexture::Layout::TransferDst);

  // ensure we don't leave this bound later on
  if (m_tfx_textures[0] == sTexVK)
    PSSetShaderResource(0, nullptr, false);

  DebugAssert((sTexVK->GetType() == GSTexture::Type::DepthStencil) ==
              (dTexVK->GetType() == GSTexture::Type::DepthStencil));
  const VkImageAspectFlags aspect =
    (sTexVK->GetType() == GSTexture::Type::DepthStencil) ? VK_IMAGE_ASPECT_DEPTH_BIT : VK_IMAGE_ASPECT_COLOR_BIT;
  const VkImageBlit ib{{aspect, sLevel, 0u, 1u},
...
bool VulkanDevice::CreatePipelineLayouts()
{
  ...
  {
    dslb.SetPushFlag();
    dslb.AddBinding(0, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, MAX_TEXTURE_SAMPLERS, VK_SHADER_STAGE_FRAGMENT_BIT);
...
void VulkanDevice::RenderBlankFrame()
{
  ...
  VulkanTexture::TransitionSubresourcesToLayout(cmdbuf, image, GPUTexture::Type::RenderTarget, 0, 1,
...
void VulkanDevice::BeginSwapChainRenderPass()
{
  ...
  VulkanTexture::TransitionSubresourcesToLayout(cmdbuf, swap_chain_image, GPUTexture::Type::RenderTarget, 0, 1,
  ...
  if (dirty != 0)
  {
    if (dirty & (DIRTY_FLAG_PIPELINE_LAYOUT | DIRTY_FLAG_DYNAMIC_OFFSETS | DIRTY_FLAG_TEXTURES_OR_SAMPLERS))
    {
      if (!UpdateDescriptorSets(dirty))
        Panic("TODO handle failure case");
    }
  }
