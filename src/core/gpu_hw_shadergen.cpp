  ...
  if (textured)
  {
    ...
    if (m_uv_limits)
    {
      DeclareFragmentEntryPoint(ss, 1, 1,
                                ...
                                true, use_dual_source ? 2 : 1, use_dual_source, !m_pgxp_depth, UsingMSAA(),
    ...
    else
    {
      DeclareFragmentEntryPoint(ss, 1, 1, {{"nointerpolation", "uint4 v_texpage"}}, true, use_dual_source ? 2 : 1,
                                use_dual_source, !m_pgxp_depth, UsingMSAA(), UsingPerSampleShading(), false,
                                m_disable_color_perspective, shader_blending);
    }
  }
  else
  {
    DeclareFragmentEntryPoint(ss, 1, 0, {}, true, use_dual_source ? 2 : 1, use_dual_source, !m_pgxp_depth, UsingMSAA(),
                              UsingPerSampleShading(), false, m_disable_color_perspective, shader_blending);
  }
...
std::string GPU_HW_ShaderGen::GenerateVRAMWriteFragmentShader(bool use_buffer, bool use_ssbo)
{
  ...
  DeclareFragmentEntryPoint(ss, 0, 1, {}, true, 1, false, true);
  ss << R"(
{
...
std::string GPU_HW_ShaderGen::GenerateVRAMCopyFragmentShader()
{
  ...
  DeclareFragmentEntryPoint(ss, 0, 1, {}, true, 1, false, true, false, false, msaa);
  ss << R"(
{
...
std::string GPU_HW_ShaderGen::GenerateVRAMFillFragmentShader(bool wrapped, bool interlaced)
{
  ...
  DeclareFragmentEntryPoint(ss, 0, 1, {}, interlaced || wrapped, 1, false, true, false, false, false);
  ss << R"(
{
